    ...
    public function widget($slug)
    {
        ...
        if ($widget) {
            if (Gate::allows('quarx', Auth::user())) {
                $widget->content .= '<a href="'.url(config('quarx.backend-route-prefix', 'quarx').'/widgets/'.$widget->id.'/edit').'" style="margin-left: 8px;position:absolute;z-index:9999" class="btn btn-xs btn-default"><span class="fa fa-pencil"></span> Edit</a>';
            }

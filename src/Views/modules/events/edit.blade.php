...
@section('content')
    ...
    <div class="row">
        @if (! is_null(request('lang')) && request('lang') !== config('cms.default-language', 'en') && $event->translationData(request('lang')))
            ...
            @else
                <a class="btn btn-default pull-right raw-margin-left-8" href="{!! url(config('cms.backend-route-prefix', 'cms').'/preview/event/'.$event->id.'?lang='.request('lang')) !!}">Preview</a>
            @endif
        ...
        @else
            @if ($event->is_published)
                <a class="btn btn-default pull-right raw-margin-left-8" href="{!! url('events/event/'.$event->id) !!}">Live</a>
            @else
                <a class="btn btn-default pull-right raw-margin-left-8" href="{!! url(config('cms.backend-route-prefix', 'cms').'/preview/event/'.$event->id) !!}">Preview</a>
            @endif
            <a class="btn btn-warning pull-right raw-margin-left-8" href="{!! Cms::rollbackUrl($event) !!}">Rollback</a>
            <a class="btn btn-default pull-right raw-margin-left-8" href="{!! url(config('cms.backend-route-prefix', 'cms').'/events/'.$event->id.'/history') !!}">History</a>
        @endif
    ...
    <div class="row raw-margin-bottom-24">
        <ul class="nav nav-tabs">
            @foreach(config('cms.languages') as $short => $language)
                <li role="presentation" @if (request('lang') == $short || is_null(request('lang')) && $short == config('cms.default-language'))) class="active" @endif><a href="{{ url(config('cms.backend-route-prefix', 'cms').'/events/'.$event->id.'/edit?lang='.$short) }}">{{ ucfirst($language) }}</a></li>
            @endforeach
        </ul>
    </div>

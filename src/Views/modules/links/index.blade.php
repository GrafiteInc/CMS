    ...
    <table class="table table-striped">
        ...
        <tbody id="linkList">
            ...
            @foreach($links as $link)
                <tr data-id="{{ $link->id }}">
                    <td><a href="{!! route(config('cms.backend-route-prefix', 'cms').'.links.edit', [$link->id]) !!}">{!! $link->name !!}</a></td>
                    @if ($link->external)
                    ...
                    <td>
                        ...
                        <a class="btn btn-xs btn-default pull-right raw-margin-right-8" href="{!! route(config('cms.backend-route-prefix', 'cms').'.links.edit', [$link->id]) !!}"><i class="fa fa-pencil"></i> Edit</a>
                    </td>
                </tr>
            @endforeach
        </tbody>

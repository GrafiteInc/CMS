    ...
    <table class="table table-striped">
        ...
        <tbody id="linkList">
            ...
            @foreach($links as $link)
                <tr data-id="{{ $link->id }}">
                    <td><a href="{!! route(config('cabin.backend-route-prefix', 'cabin').'.links.edit', [$link->id]) !!}">{!! $link->name !!}</a></td>
                    @if ($link->external)
                    ...
                    @endif
                    <td>
                        <form method="post" action="{!! url(config('cabin.backend-route-prefix', 'cabin').'/links/'.$link->id) !!}">
                            {!! csrf_field() !!}
                            {!! method_field('DELETE') !!}
                            <button class="delete-link-btn btn btn-xs btn-danger pull-right" type="submit"><i class="fa fa-trash"></i> Delete</button>
                        </form>
                        <a class="btn btn-xs btn-default pull-right raw-margin-right-8" href="{!! route(config('cabin.backend-route-prefix', 'cabin').'.links.edit', [$link->id]) !!}"><i class="fa fa-pencil"></i> Edit</a>
                    </td>
                </tr>
            @endforeach
        </tbody>
